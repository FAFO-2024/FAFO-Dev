...
function SetJobByIdentifier(identifier, jobb, grade)
    ...
    if Player then
        ...
        if Config.Framework == 'esx' or Config.Framework == 'oldesx' then
            ...
            ExecuteSql("UPDATE `mboss_employees` SET `rankLabel` = ?, `rankLevel` = ? WHERE `identifier` = ?", { tostring(jobb), tonumber(newGrade), identifier })
        else   
            local newGrade = grade or 0
            ...
            if result then
                ...
                targetJob.grade = {name = Core.Shared.Jobs[jobb].grades[tostring(newGrade)].name, level = tonumber(newGrade)}
                ExecuteSql("UPDATE `mboss_employees` SET `rankLabel` = ?, `rankLevel` = ? WHERE `identifier` = ?", { tostring(Core.Shared.Jobs[jobb].grades[tostring(newGrade)].name), tonumber(newGrade), identifier })
...
AddEventHandler("mBossmenu:fire", function(identifier)
    ...
    if company and ((CheckPlayerJob(src, targetJob) and jobLevel > targetJobLevel) or CheckIsAdmin(src)) then
        if not HasPermission(src, 'fireEmployee') and not CheckIsAdmin(src) then TriggerClientEvent("mBossmenu:sendNotification", src, "You don't have permission.") return end
...
AddEventHandler("mBossmenu:SelectRank", function(id, grade)
    ...
    if targetPlayer and company and tonumber(company.bossrank) >= tonumber(jobLevel) and company.company == job then
        if not HasPermission(src, 'recruitNewEmployee') then TriggerClientEvent("mBossmenu:sendNotification", src, "You don't have permission.") return end
