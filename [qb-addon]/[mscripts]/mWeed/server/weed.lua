...
AddEventHandler('mWeed:AddProgress', function(areaIndex, block, weedIndex, deleteItem)
    ...
    if deleteItem then
        if not CheckItem(src, {
            name = Config.Items[deleteItem].name,
            reqAmount = 1,
        })  then
            TriggerClientEvent('mWeed:SendNotification', src, string.format(Config.Notification["YOU_DONT_HAVE_ITEM"], Config.Items[deleteItem].label))
            return
        end
    end
    local isValidData = weedData[identifier] and weedData[identifier][areaIndex] and weedData[identifier][areaIndex][block] and weedData[identifier][areaIndex][block][weedIndex] 
        ...
        if deleteItem then
            ...
            if Player then
                if Config.Framework == 'esx' or Config.Framework == 'oldesx' then
                    Player.removeInventoryItem(Config.Items[deleteItem].name, 1)
                else
                    Player.Functions.RemoveItem(Config.Items[deleteItem].name, 1)
                end
            end
        end
        TriggerClientEvent('mWeed:UpdatePlayerWeedData', src, areaIndex, block, weedIndex, weedData[identifier][areaIndex][block][weedIndex])
    end
end)
...
AddEventHandler('mWeed:water', function(areaIndex, block, weedIndex)
    ...
    if not CheckItem(src, {
        ...
        TriggerClientEvent('mWeed:SendNotification', src, string.format(Config.Notifications["YOU_DONT_HAVE_ITEM"], Config.Items["water"].label))
        return
    end
    local isValidData = weedData[identifier] and weedData[identifier][areaIndex] and weedData[identifier][areaIndex][block] and weedData[identifier][areaIndex][block][weedIndex] 
    if isValidData then
...
AddEventHandler('mWeed:flounder', function(areaIndex, block, weedIndex)
    ...
    if not CheckItem(src, {
        ...
        TriggerClientEvent('mWeed:SendNotification', src, string.format(Config.Notifications["YOU_DONT_HAVE_ITEM"], Config.Items["spray"].label))
        return
    end
...
AddEventHandler('mWeed:rollWeed', function(item)
    ...
    if not hasWeed or not hasPaper then
        ...
        if not hasWeed then
            missingItems = missingItems.. ' '.. Config.Items[item].label
        end
        TriggerClientEvent('mWeed:SendNotification', src, string.format(Config.Notifications["YOU_DONT_HAVE_ITEM"], missingItems))
        return
    end
    local addItems = {}
    local items = Config.GeneralSettings["rewardItems"][item].items

    for i=1, Config.GeneralSettings["rewardItems"][item].maxamount do
...
CreateThread(function()
    ...
    if Config.Framework == 'esx' or Config.Framework == 'oldesx' then
        ...
        for _,v in pairs(Config.UseableItems) do
            Core.Functions.CreateUseableItem(_, function(source)
                local Player = GetPlayer(source)
                Player.Functions.RemoveItem(_, 1)
                TriggerClientEvent("mWeed:useJoint", source, _)
            end)
        end
    end
