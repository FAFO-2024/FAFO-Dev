...
RegisterNUICallback("getAllOwnedVehiclesForPlayers", function (_, cb)
    local p = promise.new()
    TriggerCallback("snipe-menu:server:getAllVehiclesOwnedByPlayers", function(data)
        p:resolve(data)
    end)
    local data = Citizen.Await(p)
    cb(data)
...
RegisterNUICallback("removeVehicle", function (data, cb)
    local plate = data.vehicleInfo.name
    TriggerServerEvent("snipe-menu:server:removeVehicle", plate)
    cb("ok")
end)
