...
CustomPanels = {
    ...
    }, ]]
    {
        ...
        panelTitle =  'Remove Player Vehicle',
        submitUrl = 'removeVehicle', -- in this case, clicking the panel will call this url
        components =  {
            {
                type = "searchable-dropdown", -- (CHECK README FOR ALL TYPES)
                placeholder = "", --not needed
                fetchDataUrl = "getAllOwnedVehiclesForPlayers", -- data for this type:  {{id = key1, name = "value1"}, {id = key2, name = "value2"}}
                label = "Owned List",
                fillCompulsory = true, --true or false based on if required or not
                keyName = "vehicleInfo" --cannot have spaces and should always be unique for each custom panel components - used for sending data after submission
            },
        } -- if the new panel is only clickable without any inputs in it, keep this array empty
