    ...
    if jobName == "sanitation" then
        ...
        if DoesEntityExist(car) then
            garbageJobList[groupID] = {
                ...
                ["groupID"] = groupID,
                ["carlocation"] = {x = -324.54, y = -1528.15, z = 27.54},
...
RegisterNetEvent("gksphone:server:garbageNewLocation", function (groupID)
    if garbageJobList[groupID] then
        ...
        if xPlayer then
            local GroupData = exports["gksphone"]:GetJobGroupByLeader(xPlayer.PlayerData.citizenid)
            if GroupData then
...
RegisterNetEvent("gksphone:server:garbageReturnVehicle", function (groupID)
    if garbageJobList[groupID] then
        ...
        local xPlayer = Config.Core.Functions.GetPlayer(src)
        if xPlayer then
            local GroupData = exports["gksphone"]:GetGroupByMember(xPlayer.PlayerData.citizenid)
            if GroupData then
                for k, v in pairs(GroupData.GroupMembers) do
                    local groupMember = Config.Core.Functions.GetPlayerByCitizenId(v.cid)
                    if groupMember and jobData then
                        local money = jobData.Money
                        groupMember.Functions.AddMoney("bank", money, "garbage-job")

                        local phoneData = exports["gksphone"]:GetPhoneDataByCitizenID(xPlayer.PlayerData.citizenid)
                        if phoneData then
                            local NotifData = {
