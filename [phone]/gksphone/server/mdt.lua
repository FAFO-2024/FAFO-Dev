...
Config.Core.Functions.CreateCallback('gksphone:server:wantedData', function(source, cb)
	local result = MySQL.Sync.fetchAll('SELECT * FROM gksphone_wanted ORDER BY `time` DESC', {})
	cb(result)
end)

Config.Core.Functions.CreateCallback('gksphone:server:wantedDelete', function(source, cb, id)
...
Config.Core.Functions.CreateCallback('gksphone:server:personSearch', function(source, cb, search)
...
RegisterNetEvent('gksphone:server:wantedAdd', function(cid, wreason, wappear, wlastsn)
		...
		if wanted then
			...
			-- TODO: Add notification for player that person is already wanted in the database
			local notify = {
...
Config.Core.Functions.CreateCallback('gksphone:server:ScanPlate', function(source, cb, plate)
	...
	if plate ~= nil then
		...
		-- TODO: Add notification for player that plate is invalid
		local notify = {
			title = _T(phoneLang, "MDTAPP.APP_MDT_APPTITLE"), -- The title of the notification
