...
local function UpdateBlips()
    ...
    for _, v in pairs(players) do
        if v and (v.PlayerData.job.name == "police" or v.PlayerData.job.name == "sheriff" or v.PlayerData.job.name == "sasp" or v.PlayerData.job.name == "ambulance") and v.PlayerData.job.onduty then
    ...
    TriggerClientEvent("sasp:client:UpdateBlips", -1, dutyPlayers)
end
...
local function GetCurrentCops()
    ...
    local players = QBCore.Functions.GetQBPlayers()
    for _, v in pairs(players) do
        if v and v.PlayerData.job.name == "sasp" and v.PlayerData.job.onduty then
            amount += 1
        end
    end
    return amount
...
QBCore.Commands.Add("spikestrip", Lang:t("commands.place_spike"), {}, false, function(source)
    ...
    if Player.PlayerData.job.name == "sasp" and Player.PlayerData.job.onduty then
        TriggerClientEvent('sasp:client:SpawnSpikeStrip', src)
    end
end)
...
QBCore.Commands.Add("grantlicense", Lang:t("commands.license_grant"), {{name = "id", help = Lang:t('info.player_id')}, {name = "license", help = Lang:t('info.license_type')}}, true, function(source, args)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "sasp" and Player.PlayerData.job.grade.level >= Config.LicenseRank then
...
QBCore.Commands.Add("pobject", Lang:t("commands.place_object"), {{name = "type",help = Lang:t("info.poobject_object")}}, true, function(source, args)
    ...
    if Player.PlayerData.job.name == "sasp" and Player.PlayerData.job.onduty then
        if type == "cone" then
            TriggerClientEvent("sasp:client:spawnCone", src)
        elseif type == "barrier" then
            TriggerClientEvent("sasp:client:spawnBarrier", src)
        elseif type == "roadsign" then
            TriggerClientEvent("sasp:client:spawnRoadSign", src)
        elseif type == "tent" then
            TriggerClientEvent("sasp:client:spawnTent", src)
        elseif type == "light" then
            TriggerClientEvent("sasp:client:spawnLight", src)
        elseif type == "delete" then
            TriggerClientEvent("sasp:client:deleteObject", src)
        end
    else
        TriggerClientEvent('QBCore:Notify', src, Lang:t("error.on_duty_sasp_only"), 'error')
    end
end)
...
QBCore.Commands.Add("cuff", Lang:t("commands.cuff_player"), {}, false, function(source)
    ...
    local Player = QBCore.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "sasp" and Player.PlayerData.job.onduty then
        TriggerClientEvent("sasp:client:CuffPlayer", src)
    else
        TriggerClientEvent('QBCore:Notify', src, Lang:t("error.on_duty_sasp_only"), 'error')
    end
end)
...
QBCore.Commands.Add("escort", Lang:t("commands.escort"), {}, false, function(source)
    local src = source
    TriggerClientEvent("sasp:client:EscortPlayer", src)
end)
...
QBCore.Commands.Add("jail", Lang:t("commands.jail_player"), {}, false, function(source)
    ...
    local Player = QBCore.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "sasp" and Player.PlayerData.job.onduty then
        TriggerClientEvent("sasp:client:JailPlayer", src)
    else
        TriggerClientEvent('QBCore:Notify', src, Lang:t("error.on_duty_sasp_only"), 'error')
    end
end)
...
QBCore.Commands.Add("seizecash", Lang:t("commands.seizecash"), {}, false, function(source)
    ...
    local Player = QBCore.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "sasp" and Player.PlayerData.job.onduty then
        TriggerClientEvent("sasp:client:SeizeCash", src)
    else
        TriggerClientEvent('QBCore:Notify', src, Lang:t("error.on_duty_sasp_only"), 'error')
    end
end)
...
QBCore.Commands.Add("sc", Lang:t("commands.softcuff"), {}, false, function(source)
    ...
    local Player = QBCore.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "sasp" and Player.PlayerData.job.onduty then
        TriggerClientEvent("sasp:client:CuffPlayerSoft", src)
    else
        TriggerClientEvent('QBCore:Notify', src, Lang:t("error.on_duty_sasp_only"), 'error')
    end
end)
...
QBCore.Commands.Add("cam", Lang:t("commands.camera"), {{name = "camid", help = Lang:t('info.camera_id')}}, false, function(source, args)
    ...
    local Player = QBCore.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "sasp" and Player.PlayerData.job.onduty then
        TriggerClientEvent("sasp:client:ActiveCamera", src, tonumber(args[1]))
    else
        TriggerClientEvent('QBCore:Notify', src, Lang:t("error.on_duty_sasp_only"), 'error')
    end
end)
...
QBCore.Commands.Add("depot", Lang:t("commands.depot"), {{name = "price", help = Lang:t('info.impound_price')}}, false, function(source, args)
    ...
    local Player = QBCore.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "sasp" and Player.PlayerData.job.onduty then
        TriggerClientEvent("sasp:client:ImpoundVehicle", src, false, tonumber(args[1]))
    else
        TriggerClientEvent('QBCore:Notify', src, Lang:t("error.on_duty_sasp_only"), 'error')
    end
end)
...
QBCore.Commands.Add("impound", Lang:t("commands.impound"), {}, false, function(source)
    ...
    local Player = QBCore.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "sasp" and Player.PlayerData.job.onduty then
        TriggerClientEvent("sasp:client:ImpoundVehicle", src, true)
    else
        TriggerClientEvent('QBCore:Notify', src, Lang:t("error.on_duty_sasp_only"), 'error')
    end
end)
    ...
    if Player.PlayerData.job.name == "sasp" and Player.PlayerData.job.onduty then
        ...
        if OtherPlayer then
            if OtherPlayer.PlayerData.job.name == "tow" then
                OtherPlayer.Functions.AddMoney("bank", 500, "sasp-tow-paid")
...
QBCore.Commands.Add("paylawyer", Lang:t("commands.paylawyer"), {{name = "id",help = Lang:t('info.player_id')}}, true, function(source, args)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "sasp" or Player.PlayerData.job.name == "judge" then
        local playerId = tonumber(args[1])
        local OtherPlayer = QBCore.Functions.GetPlayer(playerId)
        if not OtherPlayer then return end
        if OtherPlayer.PlayerData.job.name == "lawyer" then
            OtherPlayer.Functions.AddMoney("bank", 500, "sasp-lawyer-paid")
            TriggerClientEvent('QBCore:Notify', OtherPlayer.PlayerData.source, Lang:t("success.tow_paid"), 'success')
...
QBCore.Commands.Add("anklet", Lang:t("commands.anklet"), {}, false, function(source)
    ...
    local Player = QBCore.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "sasp" and Player.PlayerData.job.onduty then
        TriggerClientEvent("sasp:client:CheckDistance", src)
    else
        TriggerClientEvent('QBCore:Notify', src, Lang:t("error.on_duty_sasp_only"), 'error')
    end
end)
...
QBCore.Commands.Add("ankletlocation", Lang:t("commands.ankletlocation"), {{name = "cid", help = Lang:t('info.citizen_id')}}, true, function(source, args)
    ...
    if Player.PlayerData.job.name == "sasp" and Player.PlayerData.job.onduty then
        ...
        if Target.PlayerData.metadata["tracker"] then
            TriggerClientEvent("sasp:client:SendTrackerLocation", Target.PlayerData.source, src)
        else
...
QBCore.Commands.Add("takedrivinglicense", Lang:t("commands.drivinglicense"), {}, false, function(source)
    ...
    local Player = QBCore.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "sasp" and Player.PlayerData.job.onduty then
        TriggerClientEvent("sasp:client:SeizeDriverLicense", source)
    else
        TriggerClientEvent('QBCore:Notify', src, Lang:t("error.on_duty_sasp_only"), 'error')
    end
end)
...
QBCore.Commands.Add("takedna", Lang:t("commands.takedna"), {{name = "id", help = Lang:t('info.player_id')}}, true, function(source, args)
    ...
    if not OtherPlayer or Player.PlayerData.job.name ~= "sasp" or not Player.PlayerData.job.onduty then return end
...
RegisterNetEvent('sasp:server:SendTrackerLocation', function(coords, requestId)
    ...
    TriggerClientEvent("sasp:client:TrackerMessage", requestId, msg, coords)
    TriggerClientEvent("qb-phone:client:addsaspAlert", requestId, alertData)
end)

QBCore.Commands.Add('911p', Lang:t("commands.sasp_report"), {{name='message', help= Lang:t("commands.message_sent")}}, false, function(source, args)
	local src = source
    local message
    ...
    for _, v in pairs(players) do
        if v and v.PlayerData.job.name == 'sasp' and v.PlayerData.job.onduty then
            ...
            TriggerClientEvent("qb-phone:client:addsaspAlert", v.PlayerData.source, alertData)
            TriggerClientEvent('sasp:client:saspAlert', v.PlayerData.source, coords, message)
        end
    end
end)
...
QBCore.Functions.CreateUseableItem("moneybag", function(source, item)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player or not Player.Functions.GetItemByName("moneybag") or not item.info or item.info == "" or Player.PlayerData.job.name == "sasp" or not Player.Functions.RemoveItem("moneybag", 1, item.slot) then return end
    Player.Functions.AddMoney("cash", tonumber(item.info.cash), "used-moneybag")
end)

-- Callbacks
QBCore.Functions.CreateCallback('sasp:server:isPlayerDead', function(_, cb, playerId)
    local Player = QBCore.Functions.GetPlayer(playerId)
    cb(Player.PlayerData.metadata["isdead"])
end)

QBCore.Functions.CreateCallback('sasp:GetPlayerStatus', function(_, cb, playerId)
    local Player = QBCore.Functions.GetPlayer(playerId)
    local statList = {}
    if Player then
...
QBCore.Functions.CreateCallback('sasp:IsSilencedWeapon', function(source, cb, weapon)
...
QBCore.Functions.CreateCallback('sasp:GetDutyPlayers', function(_, cb)
    local dutyPlayers = {}
    local players = QBCore.Functions.GetQBPlayers()
    for _, v in pairs(players) do
        if v and v.PlayerData.job.name == "sasp" and v.PlayerData.job.onduty then
...
QBCore.Functions.CreateCallback('sasp:GetImpoundedVehicles', function(_, cb)
    local vehicles = {}
...
QBCore.Functions.CreateCallback('sasp:IsPlateFlagged', function(_, cb, plate)
    local retval = false
...
QBCore.Functions.CreateCallback('sasp:GetCops', function(_, cb)
    local amount = 0
    local players = QBCore.Functions.GetQBPlayers()
    for _, v in pairs(players) do
        if v and v.PlayerData.job.name == "sasp" and v.PlayerData.job.onduty then
...
QBCore.Functions.CreateCallback('sasp:server:IssaspForcePresent', function(_, cb)
    local retval = false
    local players = QBCore.Functions.GetQBPlayers()
    for _, v in pairs(players) do
        if v and v.PlayerData.job.name == "sasp" and v.PlayerData.job.grade.level >= 2 then
            retval = true
            break
        end
    end
...
RegisterNetEvent('sasp:server:saspAlert', function(text)
    ...
    for _, v in pairs(players) do
        if v and v.PlayerData.job.name == 'sasp' and v.PlayerData.job.onduty then
            ...
            TriggerClientEvent("qb-phone:client:addsaspAlert", v.PlayerData.source, alertData)
            TriggerClientEvent('sasp:client:saspAlert', v.PlayerData.source, coords, text)
        end
    end
end)

RegisterNetEvent('sasp:server:TakeOutImpound', function(plate, garage)
    local src = source
...
RegisterNetEvent('sasp:server:CuffPlayer', function(playerId, isSoftcuff)
    local src = source
    local playerPed = GetPlayerPed(src)
    ...
    if not Player or not CuffedPlayer or (not Player.Functions.GetItemByName("handcuffs") and Player.PlayerData.job.name ~= "sasp") then return end

    TriggerClientEvent("sasp:client:GetCuffed", CuffedPlayer.PlayerData.source, Player.PlayerData.source, isSoftcuff)
end)

RegisterNetEvent('sasp:server:EscortPlayer', function(playerId)
    local src = source
    local playerPed = GetPlayerPed(src)
    local targetPed = GetPlayerPed(playerId)
    local playerCoords = GetEntityCoords(playerPed)
    ...
    if (Player.PlayerData.job.name == "police" or Player.PlayerData.job.name == "sheriff" or Player.PlayerData.job.name == "sasp" or Player.PlayerData.job.name == "ambulance") or (EscortPlayer.PlayerData.metadata["ishandcuffed"] or EscortPlayer.PlayerData.metadata["isdead"] or EscortPlayer.PlayerData.metadata["inlaststand"]) then
        TriggerClientEvent("sasp:client:GetEscorted", EscortPlayer.PlayerData.source, Player.PlayerData.source)
    else
        TriggerClientEvent('QBCore:Notify', src, Lang:t("error.not_cuffed_dead"), 'error')
    end
end)

RegisterNetEvent('sasp:server:KidnapPlayer', function(playerId)
    local src = source
    local playerPed = GetPlayerPed(src)
    local targetPed = GetPlayerPed(playerId)
    local playerCoords = GetEntityCoords(playerPed)
    ...
    if EscortPlayer.PlayerData.metadata["ishandcuffed"] or EscortPlayer.PlayerData.metadata["isdead"] or EscortPlayer.PlayerData.metadata["inlaststand"] then
        TriggerClientEvent("sasp:client:GetKidnappedTarget", EscortPlayer.PlayerData.source, Player.PlayerData.source)
        TriggerClientEvent("sasp:client:GetKidnappedDragger", Player.PlayerData.source, EscortPlayer.PlayerData.source)
    else
        TriggerClientEvent('QBCore:Notify', src, Lang:t("error.not_cuffed_dead"), 'error')
    end
end)

RegisterNetEvent('sasp:server:SetPlayerOutVehicle', function(playerId)
    local src = source
    local playerPed = GetPlayerPed(src)
    local targetPed = GetPlayerPed(playerId)
    local playerCoords = GetEntityCoords(playerPed)
    ...
    if EscortPlayer.PlayerData.metadata["ishandcuffed"] or EscortPlayer.PlayerData.metadata["isdead"] then
        TriggerClientEvent("sasp:client:SetOutVehicle", EscortPlayer.PlayerData.source)
    else
        TriggerClientEvent('QBCore:Notify', src, Lang:t("error.not_cuffed_dead"), 'error')
    end
end)

RegisterNetEvent('sasp:server:PutPlayerInVehicle', function(playerId)
    local src = source
    local playerPed = GetPlayerPed(src)
    local targetPed = GetPlayerPed(playerId)
    local playerCoords = GetEntityCoords(playerPed)
    ...
    if EscortPlayer.PlayerData.metadata["ishandcuffed"] or EscortPlayer.PlayerData.metadata["isdead"] then
        TriggerClientEvent("sasp:client:PutInVehicle", EscortPlayer.PlayerData.source)
    else
        TriggerClientEvent('QBCore:Notify', src, Lang:t("error.not_cuffed_dead"), 'error')
    end
end)

RegisterNetEvent('sasp:server:BillPlayer', function(playerId, price)
    local src = source
    local playerPed = GetPlayerPed(src)
    local targetPed = GetPlayerPed(playerId)
    local playerCoords = GetEntityCoords(playerPed)
    ...
    local Player = QBCore.Functions.GetPlayer(src)
    local OtherPlayer = QBCore.Functions.GetPlayer(playerId)
    if not Player or not OtherPlayer or Player.PlayerData.job.name ~= "sasp" then return end

    OtherPlayer.Functions.RemoveMoney("bank", price, "paid-bills")
    exports['qb-management']:AddMoney("sasp", price)
    TriggerClientEvent('QBCore:Notify', OtherPlayer.PlayerData.source, Lang:t("info.fine_received", {fine = price}))
end)

RegisterNetEvent('sasp:server:JailPlayer', function(playerId, time)
    local src = source
    local playerPed = GetPlayerPed(src)
    local targetPed = GetPlayerPed(playerId)
    local playerCoords = GetEntityCoords(playerPed)
    ...
    TriggerClientEvent("sasp:client:SendToJail", OtherPlayer.PlayerData.source, time)
    TriggerClientEvent('QBCore:Notify', src, Lang:t("info.sent_jail_for", {time = time}))
end)

RegisterNetEvent('sasp:server:SetHandcuffStatus', function(isHandcuffed)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if Player then
...
-- RegisterNetEvent('sasp:server:FlaggedPlateTriggered', function(camId, plate, street1, street2, blipSettings)
--     local src = source
...
--             if (Player.PlayerData.job.name == "sasp" and Player.PlayerData.job.onduty) then
--                 if street2 then
--                     TriggerClientEvent("112:client:SendsaspAlert", v, "flagged", {
--                         camId = camId,
--                         plate = plate,
...
RegisterNetEvent('sasp:server:SearchPlayer', function(playerId)
    local src = source
    local playerPed = GetPlayerPed(src)
    local targetPed = GetPlayerPed(playerId)
    local playerCoords = GetEntityCoords(playerPed)
...
RegisterNetEvent('sasp:server:SeizeCash', function(playerId)
    local src = source
    local playerPed = GetPlayerPed(src)
    local targetPed = GetPlayerPed(playerId)
    local playerCoords = GetEntityCoords(playerPed)
    ...
    SearchedPlayer.Functions.RemoveMoney("cash", moneyAmount, "sasp-cash-seized")
...
RegisterNetEvent('sasp:server:SeizeDriverLicense', function(playerId)
    local src = source
    local playerPed = GetPlayerPed(src)
    local targetPed = GetPlayerPed(playerId)
    local playerCoords = GetEntityCoords(playerPed)
...
RegisterNetEvent('sasp:server:RobPlayer', function(playerId)
    local src = source
    local playerPed = GetPlayerPed(src)
    local targetPed = GetPlayerPed(playerId)
    local playerCoords = GetEntityCoords(playerPed)
    ...
    Player.Functions.AddMoney("cash", money, "sasp-player-robbed")
    SearchedPlayer.Functions.RemoveMoney("cash", money, "sasp-player-robbed")
...
RegisterNetEvent('sasp:server:UpdateBlips', function()
    -- KEEP FOR REF BUT NOT NEEDED ANYMORE.
end)

RegisterNetEvent('sasp:server:spawnObject', function(type)
    local src = source
    local objectId = CreateObjectId()
    Objects[objectId] = type
    TriggerClientEvent("sasp:client:spawnObject", src, objectId, type, src)
end)

RegisterNetEvent('sasp:server:deleteObject', function(objectId)
    TriggerClientEvent('sasp:client:removeObject', -1, objectId)
end)

RegisterNetEvent('sasp:server:Impound', function(plate, fullImpound, price, body, engine, fuel)
    local src = source
...
RegisterNetEvent('sasp:server:UpdateCurrentCops', function()
    local amount = 0
    local players = QBCore.Functions.GetQBPlayers()
    ...
    TriggerClientEvent("sasp:SetCopCount", -1, amount)
    updatingCops = false
end)
...
RegisterNetEvent('sasp:server:showFingerprint', function(playerId)
    local src = source
    TriggerClientEvent('sasp:client:showFingerprint', playerId, src)
    TriggerClientEvent('sasp:client:showFingerprint', src, playerId)
end)

RegisterNetEvent('sasp:server:showFingerprintId', function(sessionId)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    local fid = Player.PlayerData.metadata["fingerprint"]
    TriggerClientEvent('sasp:client:showFingerprintId', sessionId, fid)
    TriggerClientEvent('sasp:client:showFingerprintId', src, fid)
end)

RegisterNetEvent('sasp:server:SetTracker', function(targetId)
    local src = source
    local playerPed = GetPlayerPed(src)
    ...
    if TrackerMeta then
        ...
        TriggerClientEvent('sasp:client:SetTracker', targetId, false)
    else
        ...
        TriggerClientEvent('sasp:client:SetTracker', targetId, true)
    end
end)

RegisterNetEvent('sasp:server:SyncSpikes', function(table)
    TriggerClientEvent('sasp:client:SyncSpikes', -1, table)
end)
...
CreateThread(function()
    while true do
        ...
        TriggerClientEvent("sasp:SetCopCount", -1, curCops)
    end
end)
